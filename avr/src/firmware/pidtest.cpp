...
int main() {
    ...
    while(1) {
        ...
        sinceOut = curMillis - lastOut;

        if (sinceOut >= outPeriod) {
            lastOut += outPeriod;
            proc = true;
            ...
            reset = reset || !run || coast || brake;

            //printf("%ld, %ld, %ld", lastOut, curMillis, sinceOut);
        ...
        if (!proc) {
            ...
            if (updateReg[R_OUTMIN]) {
                //putchar('m');
                updateReg[R_OUTMIN] = false;
                //pidL.setOutputLimits(outMin, outMax);
            }
            
            if (updateReg[R_OUTMAX]) {
                //putchar('M');
                updateReg[R_OUTMAX] = false;
                //pidL.setOutputLimits(outMin, outMax);
            }
